...
public class Fire_Second : MonoBehaviour {
	...
	void Update () {
        ...
        if (length >= 1.0f)
        {
            ...
            transform.localPosition = new Vector3(0.5f, 0.47f, 0f);
    ...
    public void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.transform.tag.CompareTo("Enemy") == 0 && m_hit == false)
        {
            ...
            collision.GetComponent<K>().m_audio.loop = false;
            collision.GetComponent<K>().m_audio.clip = Resources.Load("Sounds/Injured") as AudioClip;
            collision.GetComponent<K>().m_audio.Play();
            enemy.m_stiffnessTime = 2.3f;
            enemy.m_injuredState = State.Injured_03;
